...
SPRING_BOOT_3_BASELINES = {
    "org.apache.tomcat.embed:": "10.1.40",
...
def fetch_repo_context_node(state: FlowState) -> FlowState:
    ...
    if not files and repo:
        files = (
            github.fetch_repo_files_by_paths(repo, branch, file_paths)
            if file_paths
            else github.fetch_analysis_files(repo, branch)
        )
...
def _pick_target_version_for_issue(issue: dict[str, Any], preferred_major: str | None = None) -> str | None:
    ...
    if preferred_major:
        required_preferred_major: list[str] = []
        for vulnerability in vulnerabilities:
            preferred_candidates = [
                version
                for version in vulnerability.get("fixed_versions", [])
                if _version_major(version) == preferred_major
            ]
            if not preferred_candidates:
                return None
            required_preferred_major.append(sorted(_unique(preferred_candidates), key=_version_key)[-1])
        if required_preferred_major:
            return sorted(_unique(required_preferred_major), key=_version_key)[-1]
...
def _spring_boot_baseline_version(parent_target_version: str | None, package_name: str) -> str | None:
    if _version_major(parent_target_version) != "3":
        return None
    for prefix, version in SPRING_BOOT_3_BASELINES.items():
        if package_name.startswith(prefix):
            return version
    return None


def _preferred_major_after_parent_upgrade(
    issue: dict[str, Any],
    spring_boot_parent_upgrade: dict[str, str] | None,
    package_name: str,
) -> str | None:
    if not spring_boot_parent_upgrade or _version_major(spring_boot_parent_upgrade.get("target_version")) != "3":
        return None
    if package_name.startswith("org.springframework:"):
        return "6"
    if package_name.startswith("org.apache.tomcat.embed:"):
        return "10"

    current_major = _version_major(str(issue.get("installed_version") or ""))
    if not current_major:
        return None

    higher_majors = sorted(
        {
            version_major
            for version_major in (_version_major(version) for version in issue.get("fixed_versions", []))
            if version_major and int(version_major) > int(current_major)
        },
        key=int,
    )
    return higher_majors[0] if higher_majors else None
...
def _issue_is_resolved_by_version(issue: dict[str, Any], effective_version: str) -> bool:
    effective_major = _version_major(effective_version)
    ...
    if not vulnerabilities:
        ...
        return bool(target_version and _version_major(target_version) == effective_major and _version_key(effective_version) >= _version_key(target_version))

    for vulnerability in vulnerabilities:
        fixed_versions = list(vulnerability.get("fixed_versions", []))
        same_major_candidates = [
            version for version in fixed_versions if _version_major(version) == effective_major
        ]
        if same_major_candidates:
            minimum_fix = sorted(_unique(same_major_candidates), key=_version_key)[0]
            if _version_key(effective_version) < _version_key(minimum_fix):
                return False
            continue

        higher_major_exists = any(
            version_major and int(version_major) > int(effective_major)
            for version_major in (_version_major(version) for version in fixed_versions)
        )
        if higher_major_exists:
            return False
...
def _issue_is_resolved_in_pom(pom: dict[str, Any], issue: dict[str, Any]) -> bool:
    ...
    if effective_version:
        return _issue_is_resolved_by_version(issue, effective_version)

    if (parent.get("group_id"), parent.get("artifact_id")) == SPRING_BOOT_PARENT and _is_spring_boot_bom_managed_issue(package_name, dependency):
        baseline_version = _spring_boot_baseline_version(str(parent.get("version") or ""), package_name)
        if baseline_version:
            return _issue_is_resolved_by_version(issue, baseline_version)

    return False


def _extract_issues_from_trivy_results(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped: dict[tuple[str, str, str], dict[str, Any]] = {}
    for result in results:
        target_file = str(result.get("Target") or "")
        for vulnerability in result.get("Vulnerabilities") or []:
            package_name = str(vulnerability.get("PkgName") or vulnerability.get("PkgID") or "").strip()
            if not package_name:
                continue
            ...
            fixed_versions = _fixed_versions(str(vulnerability.get("FixedVersion") or ""))
            cve_id = str(vulnerability.get("VulnerabilityID") or "").upper()
            key = (target_file, package_name, installed_version)
            item = grouped.setdefault(
                key,
                {
                    "issue_category": "VA",
            ...
            item["fixed_versions"].extend(fixed_versions)
            if cve_id:
                item["cve_ids"].append(cve_id)
            item["vulnerabilities"].append(
                {
                    "cve_id": cve_id,
                    "severity": severity,
        ...
        issues.append(
            {
                ...
                "vulnerabilities": item["vulnerabilities"],
                "detailed_issue": f"{item['package_name']} in {item['target_file']} is vulnerable.",
                "initial_fix": "Detected by iterative Trivy rescan.",
            }
        )
    return issues


def _rescan_issues_with_trivy(files: dict[str, str]) -> tuple[list[dict[str, Any]], str] | None:
    trivy_bin = os.getenv("TRIVY_BIN", "trivy").strip() or "trivy"
    if not shutil.which(trivy_bin):
        return None

    with tempfile.TemporaryDirectory(prefix="langgraph-rescan-") as temp_dir:
        root = Path(temp_dir)
        ...
        issues = _extract_issues_from_trivy_results(list(payload.get("Results") or []))
        return issues, "trivy"


def _semantic_rescan_issues(
    files: dict[str, str],
    issues: list[dict[str, Any]],
    resolved_paths: list[dict[str, Any]],
) -> list[dict[str, Any]]:
...
def _issue_signature(issue: dict[str, Any]) -> tuple[str, tuple[str, ...], str]:
    return (
        str(issue.get("package_name", "") or ""),
        tuple(sorted(str(cve) for cve in issue.get("cve_ids", []) if cve)),
        str(issue.get("installed_version", "") or ""),
    )


def rescan_vulnerabilities_node(state: FlowState) -> FlowState:
    ...
    trivy_rescan = _rescan_issues_with_trivy(files)
    if trivy_rescan is not None:
        remaining_issues, scan_mode = trivy_rescan
    else:
        remaining_issues = _semantic_rescan_issues(files, issues, state.get("resolved_paths", []))
        scan_mode = "semantic"

    current_va_issues = [issue for issue in issues if str(issue.get("issue_category", "")).upper() == "VA"]
    current_signatures = {_issue_signature(issue) for issue in current_va_issues}
    remaining_signatures = {_issue_signature(issue) for issue in remaining_issues}
    retry_requested = bool(remaining_issues) and current_signatures != remaining_signatures and iteration_count < 3
...
def validate_and_prepare_changes_node(state: FlowState) -> FlowState:
    ...
    for path, issue_indexes in path_issue_map.items():
        ...
        for issue in path_issues:
            ...
            current_version = issue.get("installed_version")
            preferred_major = _preferred_major_after_parent_upgrade(issue, spring_boot_parent_upgrade, package_name)
            target_version = _pick_target_version_for_issue(issue, preferred_major=preferred_major) or _pick_target_version_for_issue(issue)
            ...
            if spring_boot_parent_upgrade and has_spring_boot_parent and _is_spring_boot_bom_managed_issue(package_name, dependency):
                baseline_version = _spring_boot_baseline_version(spring_boot_parent_upgrade.get("target_version"), package_name)
                if baseline_version and _issue_is_resolved_by_version(issue, baseline_version):
                    continue
...
def build_graph():
    ...
    graph.add_node("rescan_vulnerabilities", rescan_vulnerabilities_node)
    ...
    graph.add_edge("validate_and_prepare_changes", "rescan_vulnerabilities")
